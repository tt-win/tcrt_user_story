 ...
 *
 * Lazy-loads on tab show — initial render of team-management does NOT hit
...
(function () {
  ...
  function init() {
    ...
    // Super Admin only — hide the tab entry for everyone else (Admin can
    // still open the org-sync modal for personnel/org/service/MCP tabs).
    // The system API already enforces this server-side; this just makes
    // the UI consistent with the access model.
  ...
  function hideTab(tabBtn) {
    // Hide the <li> wrapper so the tab doesn't take any layout space.
    const li = tabBtn.closest('li.nav-item');
    if (li) li.classList.add('d-none');
    else tabBtn.classList.add('d-none');
    // Also hide the pane content in case any other code shows it
    // programmatically.
    const pane = document.getElementById('tab-pane-org-automation-infra');
