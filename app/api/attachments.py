...
async def upload_testcase_attachment(
    ...
    try:
        ...
        if not file_content:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="檔案內容不能為空")
        ...
        if success:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="檔案上傳失敗")

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"檔案上傳過程發生錯誤: {str(e)}")
...
async def upload_testrun_attachment(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="無效的記錄 ID 格式")
    ...
    try:
        ...
        base_dir = get_attachments_root_dir(project_root)
        ...
        item_meta = {
            ...
            "relative_path": str(stored_path.relative_to(base_dir).as_posix()),
...
async def upload_file_get_token(team_id: int, file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        if file_token:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="檔案上傳到 Lark Drive 失敗")
...
async def attach_file_token_to_testcase(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="file_token 附加失敗")

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"附加 file_token 過程發生錯誤: {str(e)}"
        )


@router.delete("/teams/{team_id}/testcases/{record_id}/attachments/{file_token}")
async def remove_testcase_attachment(
    team_id: int, record_id: str, file_token: str, field_name: str = "Attachment", db: AsyncSession = Depends(get_db)
):
    """
    從測試案例記錄中移除指定的附件
    ...
    try:
        ...
        if not isinstance(existing_attachments, list):
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="附件欄位格式錯誤")
        ...
        if len(remaining_file_tokens) == len(existing_attachments):
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"找不到要刪除的附件 file_token: {file_token}"
            )
        ...
        if success:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="附件移除失敗")

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"移除附件過程發生錯誤: {str(e)}")
