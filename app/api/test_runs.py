...
def filter_test_runs(records: List[dict], **filters) -> List[dict]:
    ...
    if filters.get("assignee_filter"):
        ...
        filtered_records = [
            ...
            for r in filtered_records
            if any(
                assignee_filter in assignee.get("name", "").lower()
        ...
        for record in paginated_records:
            try:
                ...
                test_run_response = TestRunResponse(
                    ...
                    updated_at=(
                        datetime.fromtimestamp(record.get("updated_time", 0) / 1000)
                        if record.get("updated_time")
                        else None
                    ),
        ...
        if test_run.assignee_email:
            ...
            if user_info:
                ...
                test_run_instance.assignee = LarkUser(
                    id=user_info["id"], name=user_info["name"], email=user_info["email"]
                )
