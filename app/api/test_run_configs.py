...
async def get_test_run_configs(
    ...
    def _list_configs(sync_db: Session):
        ...
        query = sync_db.query(TestRunConfigDB).filter(TestRunConfigDB.team_id == team_id)
...
async def update_test_run_config(
    ...
    notify_chat_ids_json = update_result["notify_chat_ids_json"]
    if old_status != new_status and update_result["notifications_enabled"]:
        # 確認有設定通知群組
        if notify_chat_ids_json:
            try:
                chat_ids = json.loads(notify_chat_ids_json)
                if chat_ids:
...
async def sync_test_run_config(
    ...
    def _sync(sync_db: Session):
        ...
        # 以本地 items 統計
        items_q = sync_db.query(TestRunItemDB).filter(TestRunItemDB.config_id == config_id)
...
async def get_tp_search_statistics(
    ...
    Returns:
        Dict: TP 票號搜尋統計資訊
    """
    def _stats(sync_db: Session):
        # 驗證團隊存在
        verify_team_exists(team_id, sync_db)

        # 查詢該團隊的 TP 票號統計
        total_configs = sync_db.query(TestRunConfigDB).filter(
            TestRunConfigDB.team_id == team_id
        ).count()

        configs_with_tp = sync_db.query(TestRunConfigDB).filter(
            TestRunConfigDB.team_id == team_id,
            TestRunConfigDB.tp_tickets_search.isnot(None),
        ...
        # 取得所有 TP 票號進行分析
        configs_db = sync_db.query(TestRunConfigDB).filter(
            TestRunConfigDB.team_id == team_id,
            TestRunConfigDB.tp_tickets_search.isnot(None)
        ).all()
