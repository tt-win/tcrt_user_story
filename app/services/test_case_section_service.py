...
class TestCaseSectionService:
    ...
    async def create(self, test_case_set_id: int, name: str, description: str = None,
        ...
        def _create(sync_db: Session) -> TestCaseSection:
            # 驗證 parent section 存在
            if parent_section_id:
                parent = self._get_by_id_sync(sync_db, parent_section_id)
                if not parent:
            ...
            # 檢查同層級名稱唯一性
            existing = sync_db.query(TestCaseSection).filter(
                and_(
            ...
            max_sort = sync_db.query(func.max(TestCaseSection.sort_order)).filter(
                and_(
                    TestCaseSection.test_case_set_id == test_case_set_id,
                    TestCaseSection.parent_section_id == parent_section_id
                )
    ...
    async def get_by_id(self, section_id: int) -> TestCaseSection:
        """根據 ID 取得 Section"""
        def _get(sync_db: Session) -> TestCaseSection:
            return sync_db.query(TestCaseSection).filter(
    ...
    async def get_by_set(self, test_case_set_id: int) -> list[TestCaseSection]:
        ...
        def _get(sync_db: Session) -> list[TestCaseSection]:
            return (
    ...
    async def get_tree_structure(self, test_case_set_id: int) -> list:
        """取得 Section 樹狀結構"""
        def _get_tree(sync_db: Session) -> list:
    ...
    async def update(self, section_id: int, name: str = None, description: str = None) -> TestCaseSection:
        """更新 Section"""
        def _update(sync_db: Session) -> TestCaseSection:
            section = self._get_by_id_sync(sync_db, section_id)
            if not section:
    ...
    async def delete(self, section_id: int) -> bool:
        ...
        def _delete(sync_db: Session) -> bool:
            ...
            # 取得所有子 Sections ID
            child_ids = self._get_all_child_section_ids_sync(sync_db, section_id)
            all_ids = [section_id] + child_ids

            # 將這些 Sections 中的 Test Cases 移到 Unassigned
            unassigned = sync_db.query(TestCaseSection).filter(
                and_(
                    TestCaseSection.test_case_set_id == section.test_case_set_id,
    ...
    async def reorder(self, test_case_set_id: int, orders: list[dict]) -> bool:
        ...
        """
        def _reorder(sync_db: Session) -> bool:
            for order_item in orders:
                section = self._get_by_id_sync(sync_db, order_item['id'])
                ...
                if new_parent_id != section.parent_section_id:
                    if new_parent_id is not None:
                        if not self._can_move_to_parent_sync(sync_db, section.id, new_parent_id, test_case_set_id):
                            raise ValueError(f"無法移動 Section {section.id} 到父 Section {new_parent_id}")
                        parent = self._get_by_id_sync(sync_db, new_parent_id)
                        if not parent:
    ...
    async def move(self, section_id: int, new_parent_id: int = None) -> TestCaseSection:
        ...
        def _move(sync_db: Session) -> TestCaseSection:
            section = self._get_by_id_sync(sync_db, section_id)
            if not section:
                raise ValueError(f"Section 不存在: {section_id}")
            ...
            if new_parent_id:
                ...
                # 檢查層級限制和防止循環參考
                if not self._can_move_to_parent_sync(sync_db, section_id, new_parent_id, section.test_case_set_id):
    ...
    def _get_by_id_sync(self, sync_db: Session, section_id: int) -> TestCaseSection:
        return sync_db.query(TestCaseSection).filter(
            TestCaseSection.id == section_id
        ).first()

    def _get_test_case_count_sync(self, sync_db: Session, section_id: int) -> int:
    ...
    def _get_all_child_section_ids_sync(self, sync_db: Session, section_id: int) -> list:
        """遞迴取得所有子 Sections ID"""
        child_ids = []
        children = sync_db.query(TestCaseSection).filter(
            TestCaseSection.parent_section_id == section_id
        ).all()

        for child in children:
            child_ids.append(child.id)
            child_ids.extend(self._get_all_child_section_ids_sync(sync_db, child.id))
    ...
    def _update_child_levels_sync(self, sync_db: Session, section_id: int) -> None:
        """根據父節點層級遞迴更新所有子節點層級"""
        root = self._get_by_id_sync(sync_db, section_id)
        if not root:
            return
    ...
    def _can_move_to_parent_sync(self, sync_db: Session, section_id: int, new_parent_id: int, test_case_set_id: int) -> bool:
        ...
        # 檢查新父是否為此 Section 的子
        descendants = self._get_all_child_section_ids_sync(sync_db, section_id)
        if new_parent_id in descendants:
            return False

        # 檢查層級限制
        new_parent = self._get_by_id_sync(sync_db, new_parent_id)
