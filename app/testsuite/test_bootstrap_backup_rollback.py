...
The default ``BOOTSTRAP_BACKUP_MODE=required`` path is covered with SQLite so the
tests verify that a pending upgrade gets a restorable pre-upgrade snapshot.
"""
...
def test_pending_target_upgrades_with_pre_upgrade_backup(monkeypatch, tmp_path) -> None:
    """A pending existing target is backed up before the upgrade."""
    paths = _configure_env(monkeypatch, tmp_path)
    ...
    assert exit_code == 0
    assert len(_backup_files(paths["backup_dir"], "usm")) == 1
...
def test_upgrade_failure_rolls_back_with_pre_upgrade_backup(monkeypatch, tmp_path) -> None:
    """Rollback policy restores the pre-upgrade snapshot after migration failure."""
    paths = _configure_env(monkeypatch, tmp_path)
...
def test_multi_target_failure_restores_earlier_success(monkeypatch, tmp_path) -> None:
    """A later target failure restores earlier targets upgraded in this run."""
    paths = _configure_env(monkeypatch, tmp_path)
    monkeypatch.setenv("BOOTSTRAP_ON_FAILURE", "rollback")
