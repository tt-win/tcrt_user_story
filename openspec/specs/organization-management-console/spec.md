...
## Purpose
定義獨立的「組織與系統設定」頁面（`/organization-management`）——統整人員管理、組織同步、Service 管理（排程服務）、MCP Token 簽發、組織自動化基礎設施五個原本擠在 `/team-management` 頁面「組織與系統設定」modal 內的組織層分頁，對應既有 `organization` permission page key，並提供 `/team-management` 到本頁的入口連結與舊分頁錨點的相容性提示。由 `redesign-team-settings-information-architecture` change 建立。
...
### Requirement: Organization management page hosts the five relocated tabs

`/organization-management` SHALL 內建 5 個分頁，各自沿用其原本在 team_management「組織與系統設定」modal 內的功能與存取層級，僅搬遷 DOM 掛載位置與所屬頁面：
...
各分頁呼叫的既有 API（`app/api/users.py`、`app/api/organization_sync.py`、`app/api/system_automation_providers.py`、`app/api/system_automation_hub.py` 等）路徑與 request/response contract SHALL 不變。

#### Scenario: Super Admin sees all five tabs
- **WHEN** Super Admin 開啟 `/organization-management`
- **THEN** 頁面 SHALL 顯示全部 5 個分頁，且每個分頁內容與搬遷前 team_management modal 內對應分頁功能一致

#### Scenario: Admin (non-Super-Admin) sees a subset of tabs
- **WHEN** 僅具 ADMIN 角色（非 Super Admin）的使用者開啟 `/organization-management`
- **THEN** 頁面 SHALL 僅顯示人員管理與組織同步分頁，Service 管理／MCP Token／組織自動化基礎設施三個分頁 SHALL 不可見
