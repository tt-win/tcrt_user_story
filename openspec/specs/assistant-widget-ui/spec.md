...
### Requirement: 面板尺寸切換
面板 SHALL 在標題列提供展開/收起按鈕，讓使用者在 compact（小窗 390px）與 expanded（大窗 50vw）兩種尺寸之間切換。尺寸模式 SHALL 以 localStorage 跨頁面保留。expanded 模式的高度 calc 餘裕 MUST 與 compact 相同（96px），確保面板頂部不被 fixed header 遮擋。Mobile（≤575.98px）MUST 忽略 expanded 模式，維持全螢幕。按鈕的 aria-label 與 title MUST 由 JS 動態管理（不使用 `data-i18n-title`/`data-i18n-aria-label`），並在 `languageChanged` 事件時同步更新，避免 retranslate 覆蓋造成狀態不同步。

#### Scenario: compact ↔ expanded 切換
- **WHEN** 使用者點擊展開按鈕
- **THEN** 面板寬度從 390px 變為 min(50vw, 100vw-40px)，高度從 min(640px,...) 變為 min(80vh,...)，按鈕圖示與 aria-label 切換為收起

#### Scenario: 尺寸模式跨頁面保留
- **WHEN** 使用者選擇 expanded 後導航至另一頁面
- **THEN** 新頁面的助手面板以 expanded 模式開啟

#### Scenario: Mobile 不受 expanded 影響
- **WHEN** 螢幕寬度 ≤575.98px 且 panelSizeMode 為 expanded
- **THEN** 面板維持全螢幕（inset:0），不套用 expanded 尺寸

#### Scenario: 語言切換時按鈕 label 同步
- **WHEN** 使用者切換介面語言
- **THEN** 展開/收起按鈕的 aria-label 與 title 依目前尺寸模式更新為正確語系文案
