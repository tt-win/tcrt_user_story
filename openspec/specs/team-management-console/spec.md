...
## Purpose
定義縮小後的 `/team-management` 頁面正式契約——僅保留 team 資料 CRUD（含 Lark Bitable 連結）與 per-team App Token 入口，正式排除已搬遷至 `organization-management-console` 的人員管理／組織同步／Service 管理／MCP Token／組織自動化基礎設施等 org-wide 內容。由 `redesign-team-settings-information-architecture` change 建立。
