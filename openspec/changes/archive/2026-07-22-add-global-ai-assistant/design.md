...
### D12. System prompt 與 skills 的 DB 儲存與 Super Admin CRUD（Docker 安全）

**問題**：檔案制 `prompts/assistant/system.md` 與 skills 在 Docker 唯讀映像／多 replica 下無法由運維熱更新，且 LLM 對多步驟路徑需 recipe 引導。

**決策**：
- Runtime 真相 = main DB 表 `assistant_prompt_documents`（system + integer `version`）與 `assistant_skills`。
- `prompts/assistant/**` = factory seed only；`ensure_seeded` = insert-if-missing；`restore` 分 `missing-only` 與 `overwrite-builtins`（後者保留 `is_enabled`、不碰 custom）。
- Builtin skill 禁止 DELETE；可 disable／edit／reset-one。
- System PUT 樂觀鎖 + 恰好一個 `{{SKILL_CATALOG}}`；組裝時 server 注入 enabled catalog。
- Super Admin API `/api/admin/assistant/*` 與頁面 `/assistant-admin`；不依賴 assistant enabled。
- Hard safety 仍在 executor（D3/D7/D8），DB prompt 不能關閉確認。
- Agent local tools `list_skills`／`get_skill` 讀 DB enabled only；disabled ≡ 404。
- 長度上限：system 64KiB／≥200 chars；skill body 32KiB；≤200 skills。
- Audit：hash+length+preview，不傾倒全文。
- Cache：turn 開始讀 DB（可 ≤5s process cache + 寫入 invalidate）；不承諾跨 worker 0 延遲。

完整需求見 `specs/assistant-prompt-skills-admin/spec.md`。
