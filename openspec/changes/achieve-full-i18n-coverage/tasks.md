...
- [ ] 2.1 擇定外部化方案並於 `design.md` 拍板（建議：回傳錯誤碼 + 參數，前端以 i18n 翻譯；未知鍵回退顯示原始 detail）
- [ ] 2.2 建立後端訊息鍵目錄（message-key catalog）與前端對應 `errors.*` 命名空間鍵的命名規則
- [ ] 2.3 遷移 `app/api/auth.py`：將 `HTTPException.detail` 與驗證訊息改為訊息鍵，移出 linter allowlist
...
- [ ] 2.8 前端統一處理 API 錯誤回應：依錯誤鍵 + 參數以 `i18n.t()` 呈現，未知鍵回退原始 detail

## 3. Template + JS extraction（前端硬編抽出）

- [ ] 3.1 抽出整頁未接線模板 `app/templates/system_setup_standalone.html` 的文字節點為 `data-i18n`
- [ ] 3.2 抽出 `app/templates/first_login_setup.html` 的文字節點為 `data-i18n`
- [ ] 3.3 抽出高密度模板 `team_management.html` / `team_statistics.html` / `test_case_management.html` 的硬編 CJK 文字
- [ ] 3.4 將 `app/static/js/user_story_map.js` 的 `alert/confirm/showToast/showError` 字串改走 `i18n.t()`
- [ ] 3.5 將 `app/static/js/test-case-management/modal.js` 的可見字串改走 `i18n.t()`
- [ ] 3.6 處理其餘 JS 檔（含 `usm_import.js`、`test-case-drag-drop.js`）的 `alert/toast` 字面值
- [ ] 3.7 為新增的 `data-i18n` / `i18n.t()` 鍵在三語系檔補上對應字串

## 4. Missing keys & glossary（補鍵與詞彙表）

- [ ] 4.1 補齊 en-US 缺少的 78 鍵（延後：大量翻譯，linter baseline 已防回退）
- [ ] 4.2 補齊 zh-CN 缺少的 125 鍵（含整個 `adhoc.*` 命名空間）（延後：同上）
...
- [ ] 4.5 執行 linter 確認三語系鍵集合**完全**對稱（延後：待 4.1/4.2 backfill 後達成；目前 linter 以 baseline 防回退）
...
- [ ] 6.1 本機執行 i18n linter，語系鍵對稱與 CJK 字面值掃描皆通過（allowlist 已清空或僅剩明確標註的非使用者可見項）
