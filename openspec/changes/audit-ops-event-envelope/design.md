...
- **模組路徑（已決）：** `app/observability/event_catalog.py` + `app/observability/emit.py`（audit 與 ops 共用，避免分裂）。
...
**`schema_version`（已決）：nullable INTEGER。**

- 新 emit 寫 `1`。
- 歷史列保持 `NULL`。
- **禁止** `NOT NULL DEFAULT 1`（會把舊列假標成 v1）。
- Legacy 判定：`impact IS NULL`（或 `event_code IS NULL`）；`schema_version IS NULL` 亦表 pre-envelope。
...
- 新增 nullable：`event_code` VARCHAR(128)、`impact`、`outcome`、`schema_version` INTEGER NULL。
...
- **大小寫：** 以 SQLAlchemy `ilike` 為實作（與現有 username 一致）；契約保證 **ASCII** case-insensitive；非 ASCII 大小寫折叠 best-effort、跨引擎不保證一致（accepted，寫進測試以 ASCII 為主）。
...
1. audit migration（nullable 新欄）。
2. catalog + `emit_event` / `safe_emit_event` + adapter。
3. **同 PR：** WARNING audit 站點 outcome 修正。
4. search/list/export API + UI。
5. automation result ops 路徑。
...
- `schema_version`：nullable，新寫入=1。
- Emit：核心 raise + `safe_emit_event` best-effort。
- Catalog 路徑：`app/observability/`。
