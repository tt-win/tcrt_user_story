...
系統 SHALL 提供在正式 cutover 前可重複執行的 preflight/rehearsal 步驟，對 `main`、`audit`、`usm` 分別驗證 URL resolution、driver 可用性、Alembic 狀態與 legacy adoption 狀態。 The preflight MUST stop on the first unsafe target and report target-specific remediation.
...
- **WHEN** 管理者執行 preflight，且三套資料庫的 driver、URL 與 Alembic 狀態皆符合要求
- **THEN** 系統回報所有 targets ready，允許後續執行 migration 與 smoke/rehearsal

### Requirement: Migration verification SHALL 輸出一致性摘要
系統 SHALL 在 migration rehearsal 或 bootstrap 驗證完成後輸出一致性的驗證摘要，至少包含 revision、重要表存在狀態與關鍵資料檢查結果。 Verification output MUST be deterministic enough to support operator review and rollback decisions.

#### Scenario: 完成 rehearsal 後輸出摘要
- **WHEN** 管理者完成某一 target database 的 rehearsal 或 bootstrap 驗證
- **THEN** 系統輸出該 target 的 current revision、required tables 檢查結果與關鍵資料表的基本一致性資訊
