...
#### Scenario: confirm 完成後不輸出過時準備文字
- **WHEN** confirmed write 已由 `tool_finished` 回報成功，continuation 最後只回傳「已準備好／請確認」或其他純文字
- **THEN** 系統不持久化、不送出該 terminal text，只以權威狀態圖示收尾；若 continuation 實際提出新的 write tool call，仍正常建立下一張確認卡
