...
### Script 是「指標」，不是「內容主存」
- 新增 `automation_scripts` 表，儲存指向真實檔案的指標：`provider_id`、`ref_path`、`ref_branch`、`format`、`cached_content`（optional）、`cached_etag`、`last_synced_at`、`linked_test_cases`
- UI 列表 / 編輯透過 StorageProvider 即時讀取；編輯後 commit 回 git
- 不複製內容主存；script 真正活在 team 的 git repo
...
- **不做自家 CodeMirror IDE 體驗**：編輯器仍提供，但定位是「快速修改」，深度編輯仍走 VSCode + git
...
- `automation-hub-script-management`：Script 指標 CRUD + M2M linkage to test cases + 編輯體驗（delegate to StorageProvider）+ 反向 panel on test case detail
...
- **程式**：
  ...
  - 新增 templates：`automation_hub.html`（team 內 hub 入口）、`automation_script_detail.html`、`automation_run_history.html`、`automation_provider_settings.html`
