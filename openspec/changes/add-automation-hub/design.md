...
class StorageProvider(Protocol):
    ...
    async def read_script(self, path: str, ref: str | None = None) -> ScriptContent: ...
...
| `ref_path` | 在 repo 中的路徑（如 `tests/login.spec.ts`） |
...
### 6. Script 編輯流程（不 fork git workflow）

使用者在 TCRT 編輯 script content：

1. 開 script detail → TCRT 呼叫 StorageProvider.read_script → CodeMirror 顯示
2. 編輯 → 點「儲存」
3. TCRT 詢問：「commit 到 `main`？或開新分支？」
4. 若直接 commit：StorageProvider.write_script → cached_content 更新
5. 若開分支：StorageProvider.write_script(branch=...) → StorageProvider.create_pull_request → 回傳 PR URL → UI 顯示「請去 GitHub 完成 review」

**Rationale**：尊重既有 git workflow；不繞過 code review；TCRT 不嘗試取代 git。
...
- 配對：注入 `inputs.tcrt_run_id`，後續 60 秒內輪詢 `GET /repos/.../actions/runs?event=workflow_dispatch&created>=<触发时间-5s>`，過濾配對 `inputs.tcrt_run_id`
...
事件：`script.linked`、`script.unlinked`、`run.triggered`、`run.tracked`、`run.completed`
...
TCRT team 工作區內新增 **Automation Hub** 入口（與 manual test case management 並列），分四個 tab：

1. **Scripts**：列出本 team 所有 script，依 storage provider 列檔案樹 + cached metadata；列表行可點開 detail
2. **Runs**：執行歷史列表，跨 script，跨 workflow；點開直接跳 CI 原生 UI
3. **Coverage**：顯示「manual cases 自動化覆蓋率」+ 未覆蓋清單；補強 manual case detail 的反向 panel
4. **Settings**：admin 配置 storage / ci / result providers

Manual `test_case_management.html` 內每個 case detail 新增「Automation」面板：顯示 linked scripts、last run status、跳轉到報表的連結。
...
- script content cache TTL 30 秒（編輯體驗）
...
- **Air-gapped**（無外部網路）：v1 用 `storage:local_git`（mount 內部 git server 的 working copy）+ `ci:`...
  - v1 暫不附 air-gapped CI provider（Jenkins 後續另立 change）；air-gapped 場景需要 user 自己整合

### 17. 升級 / migration plan
...
- **外部依賴**：GitHub down 時 script list / 編輯不可用。對策：UI 友善降級顯示 cached content + 提示「無法連線到 GitHub」；run history 仍可看（純 TCRT DB）
...
- **編輯 vs git review 衝突**：如果使用者習慣全部走 PR 流程，TCRT 直接 commit 會繞過 review。對策：UI 預設「開分支 + PR」，「直接 commit」要 admin enable
...
2. **Script list 是 git 檔案樹自由瀏覽，還是只能在 TCRT register 過的 script 才顯示？** 傾向後者（register 後加入 `automation_scripts`），避免列太多無關檔案；register 流程簡單（admin 一次性匯入或自動掃描）
3. **要不要做自動掃描 register？** 例如 admin 設定「掃 `tests/**/*.spec.ts`」自動建立 `automation_scripts`。v1 不做自動掃描，使用者手動 register；v2 評估
4. **PR 流程整合深度**：v1 僅做「create PR」，不追蹤 PR 狀態。是否需要顯示 PR review / merge 狀態？v1 不做
5. **Cache invalidation 策略**：v1 用 TTL，未來可加 webhook（GitHub push event）即時失效
6. **Workflow inputs 對應 test data**：使用者可能在 workflow 內定義輸入（如 `environment: staging/prod`）；UI 是否要解析 workflow YAML 並提供下拉？v1 不做，使用者自填；v2 評估
